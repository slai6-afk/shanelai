/**
 * Figma Plugin API script for MCP `use_figma` (Design files only — NOT Figma Make).
 *
 * Source: README https://www.figma.com/design/RKTqjNdk8v2knPBOgQjICo/Designer-Portfolio-Website
 * Pass fileKey: RKTqjNdk8v2knPBOgQjICo (or your branch file key).
 *
 * Workflow (figma-generate-design): one call builds wrapper + both screens; return IDs for screenshots.
 * If the MCP errors with "Make files", open this file in the Figma **Design** desktop/web app, not Make.
 *
 * Paste the `run()` function BODY only into use_figma `code` — or use the whole script if your runner wraps it.
 * This file uses top-level await compatible with use_figma (async context).
 */

// --- Paste everything below into use_figma `code` field ---
